pro bin_planets, sstruct=sstruct, pstruct=pstruct, infile=infile, outfile=outfile, verbose=verbose, fov=fov, $
  ...
  template_planet = {$
 		    ...
 		    dep_eff: 0.0, $ ; Effective depth
                    dur: 0.0, $ ; Transit duration (days)
                    dur_eff: 0.0, $ ; Effective duration
...
;     (1.0 + (1.0 - 3./5.*0.541)*(2./3.*!dpi*(planet.r/3.9)^2.)^0.541)
  ; Weiss & Marcy 2014:
  lomass = where(planet.r lt 1.5)
  planet[lomass].m = 0.440*(planet[lomass].r)^3. + 0.614*(planet[lomass].r)^4.
  himass = where(planet.r ge 1.5)
  planet[himass].m = 2.69*(planet[himass].r)^0.93
; RV amplitude
