pro rad_planets, struct=struct, infile=infile,outfile=outfile, verbose=verbose

  AU_IN_RSUN = 215.093990942D0          ; in solar radii
  REARTH_IN_RSUN = 0.0091705248         ; in solar radii

  PERIOD = 10.0
  period_boundary = [1.53, 2.62, 4.47, 7.65, 13.08, 22.36, 38.24, 65.39, $
			111.82, 191.22]
...
;  psf_f = mrdfits('../OnAxisPSF.fits')
  ...
  n_planets = n_elements(star[0].planet)

  print, 'Putting ',n_planets,' planets around ',n_stars,' stars.'
...
;  star.cosi=0.0
;  star.planet_hz.cosi = star.planet.cosi
...
;  rate_fressin = rate_fressin/100.
  ...
  for ii=0,n_planets-2 do begin
    star.planet[ii].p = period_boundary[ii]
    star.planet[ii].a = (star.m)^(1./3.) * (star.planet[ii].p/365.25)^(2./3.)	; in AU
    star.planet[ii].s = (star.r)^2.0 * (star.teff/5777.0)^4. / (star.planet[ii].a)^2. ; indicent flux wrt sun-earth value
    star.planet[ii].b = (star.planet[ii].a*AU_IN_RSUN / star.r) * star.cosi; assumes circular orbit  
    star.planet[ii].tra = (abs(star.planet[ii].b) lt 1.0)	
    star.planet[ii].dur = star.r * star.planet[ii].p * sqrt(1.-(star.planet[ii].b)^2.) / (!DPI*star.planet[ii].a*AU_IN_RSUN)
  endfor

  if (v) then print, 'Done assigning period-dependent quantities'
   ii = n_planets-1
   star.planet[ii].s = 1.0
   star.planet[ii].a = star.r * (star.teff/5777.0)^2.
   star.planet[ii].p = 365.25 * sqrt(star.planet[ii].a^3. * star.m)
   
   star.planet[ii].b = (star.planet[ii].a*AU_IN_RSUN / star.r) * star.cosi; assumes circular orbit  
   star.planet[ii].tra = (abs(star.planet[ii].b) lt 1.0)	
   star.planet[ii].dur = star.r * star.planet[ii].p * sqrt(1.-(star.planet[ii].b)^2.) / (!DPI*star.planet[ii].a*AU_IN_RSUN)
 
  if (v) then print, 'Done assigning insolance-dependent quantities'
